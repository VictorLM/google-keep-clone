...
import { getArchivedNotes } from "@/app/_api/api";
...
export default function Notes() {
  ...
  async function getData(search?: string) {
    ...
    try {
      const response = await getArchivedNotes(search);
            ...
            {loading ? (
              ...
              <>
                <h4 className="text-muted fst-italic fw-light text-center p-5">
                  Nenhuma anotação arquivada encontrada!
                </h4>
              </>
            ) : (
              <>
                <div className="ms-3 mt-4">
                  <span className="fw-medium text-muted small">ARQUIVADAS</span>
                </div>
                {/* Archived notes */}
